...
const createMasterPoolUninitialized = async (
    ...
    await mainVault.grantRole(MasterVaultRoles.ROLE_NETWORK_TOKEN_MANAGER, masterPool.address);
...
const createSystemFixture = async () => {
    ...
    const mainVault = await createProxy(Contracts.MasterVault, { ctorArgs: [networkToken.address] });
    ...
    await mainVault.grantRole(MasterVaultRoles.ROLE_ASSET_MANAGER, network.address);
