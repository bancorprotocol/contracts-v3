...
describe('NetworkSettings', () => {
    const createNetworkSettings = async () => {
    ...
    describe('construction', async () => {
        it('should revert when attempting to reinitialize', async () => {
            ...
            await expect(settings.initialize()).to.be.revertedWith('Initializable: contract is already initialized');
        });
    ...
    describe('withdrawal fee', async () => {
        ...
        it('should revert when a non-owner attempts to set the withdrawal fee', async () => {
            await expect(settings.connect(nonOwner).setExitFeePPM(newExitFee)).to.be.revertedWith('ERR_ACCESS_DENIED');
        });
    ...
    describe('flash-loan fee', async () => {
        ...
        it('should be to able to set and update the flash-loan fee', async () => {
            ...
            await expect(res).to.emit(settings, 'FlashLoanFeePPMUpdated').withArgs(BigNumber.from(0), newFlashLoanFee);
            ...
            await expect(res2).to.emit(settings, 'FlashLoanFeePPMUpdated').withArgs(newFlashLoanFee, BigNumber.from(0));
    ...
    describe('maximum deviation', async () => {
        ...
        it('should revert when a non-owner attempts to set the maximum deviation', async () => {
            await expect(settings.connect(nonOwner).setAverageRateMaxDeviationPPM(newMaxDeviation)).to.be.revertedWith(
                'ERR_ACCESS_DENIED'
            );
        });
