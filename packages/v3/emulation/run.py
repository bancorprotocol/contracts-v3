...
def execute(commands):
    ...
    for command in commands:
        ...
        if command['operation'] == 'newPool':
            pool = newPool(command['swapFee'], command['numOfUsers'], command['initialAmount'])
        elif command['operation'] == 'setRates':
            pool.setRates(command['tknRate'], command['bntRate'])
        ...
        elif command['operation'] == 'swap':
            pool.swap(command['sourceToken'], command['targetToken'], command['user'], command['amount'])
