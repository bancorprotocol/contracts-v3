...
interface State {
    pools: string[];
    data: Record<string, AsyncReturnType<PoolCollectionType1V8['poolData']>>;
}
...
const savePreviousPoolData = async () => {
    ...
    const prevPoolCollection = await DeployedContracts.PoolCollectionType1V8.deployed();
...
describeDeployment(
    ...
    () => {
        ...
        let prevPoolCollection: PoolCollectionType1V8;
        let newPoolCollection: PoolCollection;

        beforeEach(async () => {
            network = await DeployedContracts.BancorNetwork.deployed();
            prevPoolCollection = await DeployedContracts.PoolCollectionType1V8.deployed();
            newPoolCollection = await DeployedContracts.PoolCollectionType1V9.deployed();
        });

        it('should deploy and migrate the new pool migration and pool collection contracts', async () => {
            expect(await newPoolCollection.version()).to.equal(9);
